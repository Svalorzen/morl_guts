...
def utility_wfh(wfh):
    ...
    # utility[wfh >= cutoff] = np.exp(scale * cutoff) / np.exp(scale * wfh[wfh >= cutoff])
    utility[wfh >= cutoff] = 1
    return utility


# utility over full time equivalent work
def utility_fte(fte):
    utility = fte ** 2
    return utility
...
# utility over personal development budget
def utility_personal_development_budget(budget):
    return budget
...
utility_functions = {
        ...
        specs_jobs.obj_fte: utility_fte,
        specs_jobs.obj_prob: utility_probation_time,
        specs_jobs.obj_budget: utility_personal_development_budget
    }
