"""
Basic experiment; illustrates how experiments can be done.
"""
import os
import matplotlib
...
start_seed = 13
num_queries = 25
# number of iterations to average the results over in each experiment
for num_iter in [10, 50, 100, 150, 200]:
    ...
    for num_obj in [5]:
        ...
        for prior_type, ref_min, ref_max in [['zero', False, False], ['linear-zero', 'full', False]]:
            ...
            for utl_noise in [0.01, 0.1]:
                ...
                for query_type in ['pairwise', 'clustering', 'ranking', 'clustering4', 'top_rank']:
                    ...
                    for iter_idx in range(num_iter):
                        ...
                        path_results_file50 = os.path.join(utils_experiment.PATH_RESULTS_DIR,
                                                           utils_parameters.get_filename(params) + '.npy')
                        params['num queries'] = num_queries
                        path_results_file = os.path.join(utils_experiment.PATH_RESULTS_DIR,
                                                         utils_parameters.get_filename(params) + '.npy')
