...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        c = random_state.uniform(1, 5, 1)[0]
        y = _poly(x, c)
        ...
        # get a user
        user = UserPreference(num_objectives=num_obj, std_noise=0, seed=seed)

        # get a CCS
        ccs = utils_ccs.get_pcs_grid(ccs_size=ccs_size, num_objectives=num_obj, min_size=30, seed=seed)
        ccs = ccs[np.argsort(ccs[:, 0])]
        user.rescale_on_input_domain(ccs)

        plt.subplot(1, 3, 3)
        plt.plot(ccs[:, 0], user.get_preference(ccs, False), 'k')
        plt.xticks([])
        plt.yticks([])
